...
module EventState
  ...
  def notifiable
    ...
    return false unless stakeholders.count.positive?
    return false unless stakeholders.all?(&:email)
  ...
  def process_acceptance(options)
    if options[:send_mail]
      event_people.stakeholder.each do |event_person|
  ...
  def process_bulk_notification(reason)
    event_people.stakeholder.each do |event_person|
      ...
      if conference.ticket_type == 'rt'
        conference.ticket_server.add_correspondence(
