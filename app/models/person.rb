class Person < ApplicationRecord
  ...
  def stakeholder_of?(conferences)
  ...
  def active_in_any_conference?
    found = Conference.joins(events: [{ event_people: :person }])
      ...
      .where(EventPerson.arel_table[:event_role].in(EventPerson::STAKEHOLDERS))
  ...
  def events_as_presenter_in(conference)
    events.where('event_people.event_role': EventPerson::STAKEHOLDERS, conference: conference)
  end

  def events_as_presenter_not_in(conference)
    events.where('event_people.event_role': EventPerson::STAKEHOLDERS).where.not(conference: conference)
  end
  ...
  def update_from_omniauth(auth)
    ...
    begin
      ...
      if avatar.exists?
        existing_avatar_data = Paperclip.io_adapters.for(pe.avatar).read
